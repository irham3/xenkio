...
import { useMorse } from '@/features/morse-code/hooks/use-morse';
import { MORSE_MODES } from '@/features/morse-code/constants';
...
export default function MorseCodeClient() {
  const { options, result, isProcessing, updateOption, process, swapInputOutput, clear } = useMorse();

  const currentMode = MORSE_MODES.find(m => m.id === options.mode);
  ...
  return (
    <div className="w-full">
      ...
      <div
        ...
        role="tablist"
        aria-label="Morse code mode selection"
      >
        {MORSE_MODES.map((mode) => (
          <button
            ...
            onClick={() => updateOption('mode', mode.id as MorseMode)}
            className={cn(
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-baseline justify-between">
                <Label htmlFor="input-text" className="text-sm font-semibold text-gray-800">
                  {options.mode === 'encode' ? 'Text to Encode' : 'Morse Code to Decode'}
                </Label>
            ...
            <div className="flex flex-col h-auto">
              ...
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-semibold text-gray-800">
                  {options.mode === 'encode' ? 'Morse Code Output' : 'Decoded Text'}
                </h3>
                  ...
                  >
                    {result?.error ? (
                      ...
                      <div className="flex flex-col items-center justify-center h-full gap-3 py-8 opacity-50">
                        ...
                        <p className="text-sm">
                          {options.mode === 'encode'
                            ? 'Enter text to see Morse code output...'
                            : 'Enter Morse code to see decoded text...'
                          }
                        </p>
                      </div>
                    )}
