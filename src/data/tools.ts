import React from "react";
import {
  Code2,
  FileCode,
  Timer,
  Mic,
  ShieldCheck,
  Share2,
  Database,
  BarChart3,
  ScanFace,
  PenLine,
  FlaskConical,
  GitBranch,
  Monitor,
  ScreenShare,
  FileSearch2,
  ShieldAlert,
  FileCheck,
  ScanText,
  Shuffle,
} from "lucide-react";

export interface ToolData {
  id: string;
  title: string;
  description: string;
  icon: string;
  href: string;
  featured?: boolean;
  isNew?: boolean;
  isComingSoon?: boolean;
  isPremium?: boolean;
  categoryId: string;
  slug?: string;
}

export const TOOLS: ToolData[] = [
  // =====================================================
  // PDF & DOCUMENT TOOLS (Tier 1 - Highest Traffic)
  // =====================================================
  {
    id: "1",
    title: "Merge PDF",
    description:
      "Combine multiple PDF files into one document. Drag to reorder and download instantly.",
    icon: "GitMerge",
    href: "/tools/merge-pdf",
    slug: "merge-pdf",
    featured: true,
    categoryId: "documents",
  },
  {
    id: "2",
    title: "Split PDF",
    description: "Split PDF documents into separate files by page range.",
    icon: "SplitHorizontal",
    href: "/tools/split-pdf",
    slug: "split-pdf",
    categoryId: "documents",
  },
  {
    id: "5",
    title: "Image to PDF",
    description: "Convert images (JPG, PNG, WebP) to PDF documents.",
    icon: "FileImage",
    href: "/tools/image-to-pdf",
    slug: "image-to-pdf",
    categoryId: "documents",
  },
  {
    id: "6",
    title: "PDF Compressor",
    description: "Reduce PDF file size while maintaining quality.",
    icon: "FileArrowDown",
    href: "/tools/pdf-compressor",
    slug: "pdf-compressor",
    isNew: true,
    categoryId: "documents",
  },
  {
    id: "58",
    title: "Protect/Unlock PDF",
    description:
      "Encrypt PDF with password or remove password security permissions.",
    icon: "Lock",
    href: "/tools/protect-unlock-pdf",
    slug: "protect-unlock-pdf",
    categoryId: "documents",
    isNew: true,
  },
  {
    id: "59",
    title: "PDF to Image",
    description: "Convert PDF pages to high-quality JPG or PNG images.",
    icon: "FileImage",
    href: "/tools/pdf-to-image",
    slug: "pdf-to-image",
    categoryId: "documents",
    isNew: true,
  },
  {
    id: "62",
    title: "Invoice Generator",
    description:
      "Create professional invoices instantly. Add items, taxes, discounts, and download as PDF.",
    icon: "Receipt",
    href: "/tools/invoice-generator",
    slug: "invoice-generator",
    isNew: true,
    categoryId: "documents",
  },
  {
    id: "67",
    title: "Sign PDF",
    description:
      "Add your digital signature to PDF documents securely in your browser.",
    icon: "FilePlus",
    href: "/tools/sign-pdf",
    slug: "sign-pdf",
    categoryId: "documents",
    isNew: true,
  },
  {
    id: "102",
    title: "Crop PDF",
    description:
      "Visually crop and trim PDF pages. Remove margins, headers, or resize page area with precision.",
    icon: "Crop",
    href: "/tools/crop-pdf",
    slug: "crop-pdf",
    categoryId: "documents",
    isNew: true,
  },
  {
    id: "3",
    title: "PDF to Word",
    description:
      "Convert PDF to Word with preserved formatting: bold, italic, colors, fonts, and layout structure.",
    icon: "FileText",
    href: "/tools/pdf-to-word",
    slug: "pdf-to-word",
    categoryId: "documents",
  },
  {
    id: "4",
    title: "Word to PDF",
    description:
      "Convert Word documents to PDF with preserved formatting and layout.",
    icon: "FilePlus",
    href: "/tools/word-to-pdf",
    slug: "word-to-pdf",
    categoryId: "documents",
  },
  {
    id: "139",
    title: "Compare PDF",
    description:
      "Compare two PDF files side-by-side with visual ghosting overlay and pixel-level diff highlighting.",
    icon: "FileSearch2",
    href: "/tools/compare-pdf",
    slug: "compare-pdf",
    isNew: true,
    categoryId: "documents",
  },
  {
    id: "138",
    title: "Stamp PDF",
    description:
      "Add professional stamps to your PDF documents — approval, date, checklist, notes, and more.",
    icon: "FileCheck",
    href: "/tools/stamp-pdf",
    slug: "stamp-pdf",
    isNew: true,
    categoryId: "documents",
  },
  {
    id: "140",
    title: "PDF Reader (AI Gestures)",
    description:
      "Read PDF documents in your browser with AI-powered hand gesture page navigation.",
    icon: "Eye",
    href: "/tools/pdf-reader",
    slug: "pdf-reader",
    isNew: true,
    categoryId: "documents",
  },

  {
    id: "7",
    title: "ZIP Extractor",
    description:
      "Extract ZIP archive files online. View contents, download individual files, or save everything instantly.",
    icon: "Archive",
    href: "/tools/zip-extractor",
    slug: "zip-extractor",
    isNew: true,
    categoryId: "documents",
  },

  // =====================================================
  // IMAGE TOOLS (Tier 1 - High Traffic)
  // =====================================================
  {
    id: "63",
    title: "Remove Background",
    description:
      "Remove image backgrounds instantly with high-precision AI. Download transparent PNGs.",
    icon: "MagicWand",
    href: "/tools/background-remover",
    slug: "background-remover",
    featured: true,
    isNew: true,
    categoryId: "media-images",
  },
  {
    id: "8",
    title: "Image Compressor",
    description:
      "Reduce image file size by up to 80% without visible quality loss. Supports JPG, PNG, WebP.",
    icon: "FileArrowDown",
    href: "/tools/image-compressor",
    slug: "image-compressor",
    featured: true,
    categoryId: "media-images",
  },
  {
    id: "100",
    title: "Video Compressor",
    description:
      "Compress MP4, WebM, MOV videos in browser. Reduce file size with adjustable quality.",
    icon: "FilmStrip",
    href: "/tools/video-compressor",
    slug: "video-compressor",
    isNew: true,
    categoryId: "media-images",
  },
  {
    id: "11",
    title: "Image Converter",
    description: "Convert between JPG, PNG, WebP, GIF, and other formats.",
    icon: "Stack",
    href: "/tools/image-converter",
    categoryId: "media-images",
    slug: "image-converter",
  },
  {
    id: "12",
    title: "Favicon Generator",
    description: "Create favicon.ico files for your website from any image.",
    icon: "FileImage",
    href: "/tools/favicon-generator",
    slug: "favicon-generator",
    categoryId: "media-images",
  },
  {
    id: "13",
    title: "Image to Base64",
    description: "Convert images to Base64 encoded strings for embedding.",
    icon: "FileCode",
    href: "/tools/image-to-base64",
    slug: "image-to-base64",
    categoryId: "media-images",
  },
  // {
  //     id: '14',
  //     title: 'SVG to PNG',
  //     description: 'Convert SVG vector graphics to PNG raster images.',
  //     icon: 'Stack',
  //     href: '/tools/svg-to-png',
  //     categoryId: 'media-images',
  // },
  {
    id: "15",
    title: "Collage Maker",
    description:
      "Create premium, seamless panoramic carousels for Instagram. SCRL-style editor.",
    icon: "Images",
    href: "/tools/instagram-carousel",
    slug: "instagram-carousel",
    isNew: true,
    categoryId: "media-images",
  },

  // =====================================================
  // QR CODE & BARCODE
  // =====================================================
  {
    id: "16",
    title: "QR Code Generator",
    description:
      "Create custom QR codes with logos, colors, and multiple export formats.",
    icon: "QrCode",
    href: "/tools/qr-code-generator",
    slug: "qr-code-generator",
    featured: true,
    categoryId: "media-images",
  },

  {
    id: "18",
    title: "Barcode Generator",
    description: "Generate barcodes in various formats (EAN, UPC, Code128).",
    icon: "Barcode",
    href: "/tools/barcode-generator",
    slug: "barcode-generator",
    categoryId: "media-images",
  },
  // =====================================================
  // IMAGE TOOLS - Tambahan
  // =====================================================
  {
    id: "66",
    title: "Image Crop & Rotate",
    description:
      "Crop, rotate, and resize images with custom aspect ratios or freehand selection.",
    icon: "Crop",
    href: "/tools/image-cropper",
    slug: "image-cropper",
    categoryId: "media-images",
    isNew: true,
  },
  {
    id: "72",
    title: "Video to GIF Converter",
    description:
      "Convert video files to animated GIFs in your browser. Trim clips, adjust FPS and resolution. No upload needed.",
    icon: "FilmStrip",
    href: "/tools/video-to-gif",
    slug: "video-to-gif",
    categoryId: "media-images",
    isNew: true,
  },
  {
    id: "17",
    title: "QR Code Reader",
    description: "Scan and decode QR codes from images or camera.",
    icon: "Scan",
    href: "/tools/qr-code-reader",
    slug: "qr-code-reader",
    categoryId: "media-images",
  },
  {
    id: "9",
    title: "Image Resizer",
    description: "Resize images to exact dimensions or by percentage.",
    icon: "ArrowsOut",
    href: "/tools/image-resizer",
    slug: "image-resizer",
    categoryId: "media-images",
  },
  {
    id: "127",
    title: "Screen Recorder",
    description:
      "Record your screen, window, or browser tab with audio. No installation, no upload, 100% private.",
    icon: "Globe",
    href: "/tools/screen-recorder",
    slug: "screen-recorder",
    categoryId: "media-images",
    isNew: true,
  },
  {
    id: "141",
    title: "Running Text / Marquee Display",
    description:
      "Create animated scrolling running text with strobe effects, blink mode, and custom colors.",
    icon: "Monitor",
    href: "/tools/running-text",
    slug: "running-text",
    isNew: true,
    categoryId: "media-images",
  },
  {
    id: "136",
    title: "Sound Recorder",
    description:
      "Record audio from your microphone directly in your browser. Pause, resume, and download as WebM or MP4. No installation, no upload, 100% private.",
    icon: "Microphone",
    href: "/tools/sound-recorder",
    slug: "sound-recorder",
    categoryId: "media-images",
    isNew: true,
  },
  {
    id: "135",
    title: "EXIF Viewer",
    description:
      "View embedded metadata from photos — camera model, settings, GPS, date and more. 100% private.",
    icon: "FileText",
    href: "/tools/exif-viewer",
    slug: "exif-viewer",
    categoryId: "media-images",
    isNew: true,
  },
  // =====================================================
  // SECURITY & PRIVACY TOOLS
  // =====================================================
  {
    id: "19",
    title: "Password Generator",
    description: "Generate secure passwords with custom complexity rules.",
    icon: "Lock",
    href: "/tools/password-generator",
    slug: "password-generator",
    featured: true,
    categoryId: "security-privacy",
  },
  {
    id: "20",
    title: "Hash Generator",
    description:
      "Generate MD5, SHA-256, bcrypt, and other hash values from text.",
    icon: "Hash",
    href: "/tools/hash-generator",
    slug: "hash-generator",
    categoryId: "security-privacy",
  },
  {
    id: "21",
    title: "UUID Generator",
    description: "Generate unique UUIDs/GUIDs in v1 through v7 formats.",
    icon: "Cpu",
    href: "/tools/uuid-generator",
    slug: "uuid-generator",
    isNew: true,
    categoryId: "security-privacy",
  },
  {
    id: "22",
    title: "Base64 Encoder",
    description: "Encode and decode Base64 strings and files.",
    icon: "Terminal",
    href: "/tools/base64-encoder",
    slug: "base64-encoder",
    categoryId: "security-privacy",
  },
  {
    id: "23",
    title: "URL Encoder",
    description: "Encode and decode URLs for safe transmission.",
    icon: "Link",
    href: "/tools/url-encoder",
    slug: "url-encoder",
    categoryId: "security-privacy",
  },
  {
    id: "24",
    title: "JWT Debugger",
    description:
      "Decode, verify signature, and generate JSON Web Tokens (JWT) securely.",
    icon: "Key",
    href: "/tools/jwt-debugger",
    slug: "jwt-debugger",
    categoryId: "security-privacy",
    isNew: true,
  },
  {
    id: "25",
    title: "2FA TOTP Authenticator",
    description:
      "Generate TOTP 2FA codes from secret keys or QR codes. Supports SHA1/SHA256/SHA512. 100% client-side.",
    icon: "Shield",
    href: "/tools/totp-authenticator",
    slug: "totp-authenticator",
    isNew: true,
    categoryId: "security-privacy",
  },
  {
    id: "130",
    title: "Checksum Checker",
    description:
      "Verify file integrity by computing MD5, SHA-1, SHA-256, SHA-512, and CRC32 checksums. Drop any file — runs entirely in your browser.",
    icon: "FilePlus",
    href: "/tools/checksum-checker",
    slug: "checksum-checker",
    isNew: true,
    categoryId: "security-privacy",
  },

  // =====================================================
  // DEVELOPER TOOLS
  // =====================================================
  {
    id: "26",
    title: "JSON Formatter",
    description:
      "Format, validate, and beautify JSON data with syntax highlighting.",
    icon: "BracketsCurly",
    href: "/tools/json-formatter",
    slug: "json-formatter",
    featured: true,
    categoryId: "developer-tools",
  },
  {
    id: "27",
    title: "JSON to CSV",
    description: "Convert JSON data to CSV format for spreadsheets.",
    icon: "FileCode",
    href: "/tools/json-to-csv",
    slug: "json-to-csv",
    isNew: true,
    categoryId: "developer-tools",
  },
  {
    id: "28",
    title: "HTML Formatter",
    description: "Format and beautify HTML code with proper indentation.",
    icon: "CodeSimple",
    href: "/tools/html-formatter",
    slug: "html-formatter",
    categoryId: "developer-tools",
  },
  {
    id: "57",
    title: "Countdown Timer",
    description: "Create a countdown timer for any date or event.",
    icon: "Clock",
    href: "/tools/timer",
    slug: "timer",
    isNew: true,
    categoryId: "developer-tools",
  },
  {
    id: "147",
    title: "Data Checker",
    description:
      "Validate your CSV data manually row-by-row. Mark rows as valid or invalid, add comments, export results.",
    icon: "Database",
    href: "/tools/data-checker",
    slug: "data-checker",
    isNew: true,
    categoryId: "developer-tools",
  },
  {
    id: "148",
    title: "Data Verifier",
    description:
      "Verify and synchronize mismatched data between two sources instantly. find differences and fix errors.",
    icon: "ShieldCheck",
    href: "/tools/data-verifier",
    slug: "data-verifier",
    isNew: true,
    categoryId: "developer-tools",
  },
  {
    id: "61",
    title: "Pomodoro Timer",
    description:
      "Boost focus and productivity with customizable work intervals and breaks.",
    icon: "Timer",
    href: "/tools/pomodoro-timer",
    slug: "pomodoro-timer",
    isNew: true,
    featured: true,
    categoryId: "developer-tools",
  },
  {
    id: "33",
    title: "Markdown Editor",
    description: "Edit and preview Markdown with live rendering.",
    icon: "NotePencil",
    href: "/tools/markdown-editor",
    slug: "markdown-editor",
    categoryId: "developer-tools",
    isNew: true,
  },
  {
    id: "29",
    title: "CSS Minifier",
    description: "Minify CSS code to reduce file size.",
    icon: "FileCode",
    href: "/tools/css-minifier",
    slug: "css-minifier",
    isNew: true,
    categoryId: "developer-tools",
  },
  {
    id: "30",
    title: "JavaScript Minifier",
    description: "Minify JavaScript code to reduce file size.",
    icon: "FileCode",
    href: "/tools/js-minifier",
    slug: "js-minifier",
    isNew: true,
    categoryId: "developer-tools",
  },
  {
    id: "131",
    title: "SVG Optimizer",
    description:
      "Optimize and compress SVG files using SVGO. Remove metadata, comments, and redundant data to reduce file size.",
    icon: "FileCode",
    href: "/tools/svg-optimizer",
    slug: "svg-optimizer",
    isNew: true,
    categoryId: "developer-tools",
  },
  {
    id: "31",
    title: "Regex Tester",
    description: "Test and debug regular expressions with live matching.",
    icon: "TextT",
    href: "/tools/regex-tester",
    slug: "regex-tester",
    categoryId: "developer-tools",
  },
  {
    id: "32",
    title: "Unix Timestamp",
    description: "Convert between Unix timestamps and human-readable dates.",
    icon: "Clock",
    href: "/tools/unix-timestamp",
    slug: "unix-timestamp",
    categoryId: "developer-tools",
  },
  {
    id: "34",
    title: "Diff Checker",
    description:
      "Compare two texts or code snippets and highlight differences.",
    icon: "GitMerge",
    href: "/tools/diff-checker",
    slug: "diff-checker",
    categoryId: "developer-tools",
  },
  {
    id: "35",
    title: "HTML Entity Encoder",
    description: "Encode and decode HTML entities.",
    icon: "CodeSimple",
    href: "/tools/html-entity-encoder",
    slug: "html-entity-encoder",
    isNew: true,
    categoryId: "developer-tools",
  },
  {
    id: "108",
    title: "SQL Formatter",
    description:
      "Format and beautify SQL queries with support for multiple dialects.",
    icon: "HardDrives",
    href: "/tools/sql-formatter",
    slug: "sql-formatter",
    isNew: true,
    categoryId: "developer-tools",
  },
  {
    id: "134",
    title: "YAML Validator",
    description:
      "Validate, format, and convert YAML in real-time. Pinpoints errors by line and column. Convert YAML ↔ JSON instantly.",
    icon: "FileCode",
    href: "/tools/yaml-validator",
    slug: "yaml-validator",
    isNew: true,
    categoryId: "developer-tools",
  },

  // =====================================================
  // DESIGN & COLOR TOOLS
  // =====================================================
  {
    id: "36",
    title: "Color Picker",
    description:
      "Pick colors from images and convert between HEX, RGB, and HSL.",
    icon: "Palette",
    href: "/tools/color-picker",
    slug: "color-picker",
    categoryId: "design-tools",
  },
  {
    id: "37",
    title: "Color Palette Generator",
    description: "Generate harmonious color palettes for your designs.",
    icon: "PaintBrush",
    href: "/tools/color-palette",
    slug: "color-palette",
    isNew: true,
    categoryId: "design-tools",
  },
  {
    id: "38",
    title: "Gradient Generator",
    description: "Create beautiful CSS gradients with visual editor.",
    icon: "Stack",
    href: "/tools/gradient-generator",
    slug: "gradient-generator",
    isNew: true,
    categoryId: "design-tools",
  },
  {
    id: "145",
    title: "Keyframe Animation Generator",
    description:
      "Create CSS keyframe animations visually with live preview and instant code export.",
    icon: "MagicWand",
    href: "/tools/keyframe-animation-generator",
    slug: "keyframe-animation-generator",
    isNew: true,
    categoryId: "design-tools",
  },
  {
    id: "146",
    title: "Glassmorphism Generator",
    description:
      "Create beautiful glassmorphism effects with live preview and CSS code export.",
    icon: "Stack",
    href: "/tools/glassmorphism-generator",
    slug: "glassmorphism-generator",
    isNew: true,
    categoryId: "design-tools",
  },
  {
    id: "39",
    title: "Contrast Checker",
    description: "Check color contrast ratio for accessibility (WCAG).",
    icon: "CircleHalf",
    href: "/tools/contrast-checker",
    slug: "contrast-checker",
    isNew: true,
    categoryId: "design-tools",
  },

  // =====================================================
  // TEXT & WRITING TOOLS
  // =====================================================
  {
    id: "60",
    title: "Speech to Text",
    description:
      "Convert your voice into text instantly with real-time transcription. Supports multiple languages.",
    icon: "Microphone",
    href: "/tools/speech-to-text",
    slug: "speech-to-text",
    isNew: true,
    categoryId: "text-utilities",
  },
  {
    id: "137",
    title: "Teleprompter",
    description:
      "Free online teleprompter with auto-scroll and reading modes. Perfect for presentations, recording, and script reading.",
    icon: "Monitor",
    href: "/tools/teleprompter",
    slug: "teleprompter",
    isNew: true,
    categoryId: "text-utilities",
  },
  {
    id: "40",
    title: "Lorem Ipsum Generator",
    description: "Generate placeholder text in various formats and lengths.",
    icon: "NotePencil",
    href: "/tools/lorem-ipsum",
    slug: "lorem-ipsum",
    categoryId: "text-utilities",
  },
  {
    id: "41",
    title: "Word Counter",
    description: "Count words, characters, sentences, and paragraphs.",
    icon: "Calculator",
    href: "/tools/word-counter",
    slug: "word-counter",
    isNew: true,
    categoryId: "text-utilities",
  },
  {
    id: "42",
    title: "Case Converter",
    description:
      "Convert text between uppercase, lowercase, title case, and more.",
    icon: "Cursor",
    href: "/tools/case-converter",
    slug: "case-converter",
    isNew: true,
    categoryId: "text-utilities",
  },
  {
    id: "142",
    title: "Text Replace",
    description:
      "Find and replace text instantly with support for plain text and regex patterns.",
    icon: "NotePencil",
    href: "/tools/text-replace",
    slug: "text-replace",
    isNew: true,
    categoryId: "text-utilities",
  },
  {
    id: "154",
    title: "Text Affixer",
    description:
      "Add prefixes and suffixes to each line or an entire text block.",
    icon: "TextT",
    href: "/tools/text-affixer",
    slug: "text-affixer",
    isNew: true,
    categoryId: "text-utilities",
  },
  {
    id: "152",
    title: "ASCII Table Generator",
    description:
      "Convert CSV, TSV, and delimited data into clean plain text ASCII tables.",
    icon: "GridFour",
    href: "/tools/ascii-table-generator",
    slug: "ascii-table-generator",
    isNew: true,
    categoryId: "text-utilities",
  },
  {
    id: "143",
    title: "Morse Code Encoder & Decoder",
    description:
      "Convert text to Morse code or decode Morse code back to text instantly.",
    icon: "Hash",
    href: "/tools/morse-code",
    slug: "morse-code",
    isNew: true,
    categoryId: "text-utilities",
  },
  // Text GitMerge removed - redundant with Diff Checker
  {
    id: "43",
    title: "Text Cleaner",
    description:
      "Remove extra spaces, duplicate lines, and clean up messy text.",
    icon: "NotePencil",
    href: "/tools/text-cleaner",
    slug: "text-cleaner",
    categoryId: "text-utilities",
  },
  {
    id: "44",
    title: "Slug Generator",
    description: "Generate URL-friendly slugs from text.",
    icon: "Link",
    href: "/tools/slug-generator",
    slug: "slug-generator",
    categoryId: "text-utilities",
  },
  {
    id: "45",
    title: "Text to Speech",
    description: "Convert text to natural-sounding speech audio.",
    icon: "SpeakerHigh",
    href: "/tools/text-to-speech",
    slug: "text-to-speech",
    categoryId: "text-utilities",
  },
  // Text Formatter removed - redundant with specific formatters

  // =====================================================
  // WEB & SEO TOOLS
  // =====================================================
  {
    id: "47",
    title: "Meta Tag Generator",
    description: "Generate SEO-optimized meta tags for your website.",
    icon: "Globe",
    href: "/tools/meta-tag-generator",
    slug: "meta-tag-generator",
    categoryId: "web-seo",
  },
  {
    id: "48",
    title: "Sitemap Generator",
    description: "Generate XML sitemaps for search engines.",
    icon: "MapTrifold",
    href: "/tools/sitemap-generator",
    slug: "sitemap-generator",
    categoryId: "web-seo",
  },
  {
    id: "49",
    title: "Robots.txt Generator",
    description: "Create robots.txt files for search engine crawlers.",
    icon: "Robot",
    href: "/tools/robots-txt-generator",
    slug: "robots-txt-generator",
    categoryId: "web-seo",
  },

  // =====================================================
  // DATA & CALCULATION TOOLS
  // =====================================================
  {
    id: "70",
    title: "Scientific Calculator",
    description:
      "Full-featured calculator with trigonometric, logarithmic, and exponential functions.",
    icon: "Calculator",
    href: "/tools/scientific-calculator",
    slug: "scientific-calculator",
    categoryId: "calculators",
  },
  {
    id: "52",
    title: "Unit Converter",
    description: "Convert between different units of measurement.",
    icon: "Calculator",
    href: "/tools/unit-converter",
    slug: "unit-converter",
    categoryId: "calculators",
  },
  {
    id: "53",
    title: "Discount Calculator",
    description:
      "Calculate the final price after discount. Supports stacked discounts and tax.",
    icon: "Percent",
    href: "/tools/discount-calculator",
    slug: "discount-calculator",
    categoryId: "calculators",
  },
  {
    id: "153",
    title: "Percentage Calculator",
    description:
      "Calculate percent of a value, percentage share, percent change, and increase or decrease by percentage.",
    icon: "Percent",
    href: "/tools/percentage-calculator",
    slug: "percentage-calculator",
    isNew: true,
    categoryId: "calculators",
  },
  {
    id: "151",
    title: "Aspect Ratio Calculator",
    description:
      "Simplify ratios and resize images, videos, and layouts while preserving proportions.",
    icon: "ArrowsOut",
    href: "/tools/aspect-ratio-calculator",
    slug: "aspect-ratio-calculator",
    isNew: true,
    categoryId: "calculators",
  },
  {
    id: "54",
    title: "Age Calculator",
    description:
      "Calculate your exact age in years, months, and days. See total days lived and countdown to your next birthday.",
    icon: "Calendar",
    href: "/tools/age-calculator",
    slug: "age-calculator",
    isNew: true,
    categoryId: "calculators",
  },
  {
    id: "93",
    title: "Date Calculator",
    description:
      "Calculate the difference between two dates or add/subtract days, weeks, months, and years from any date.",
    icon: "Calendar",
    href: "/tools/date-calculator",
    slug: "date-calculator",
    isNew: true,
    categoryId: "calculators",
  },
  {
    id: "149",
    title: "Event Timekeeper",
    description:
      "Professional event schedule manager with real-time countdown and presenter focus mode.",
    icon: "Timer",
    href: "/tools/event-timekeeper",
    slug: "event-timekeeper",
    isNew: true,
    categoryId: "calculators",
  },
  {
    id: "144",
    title: "Matrix Calculator",
    description:
      "Perform matrix operations like addition, subtraction, multiplication, and determinant calculation.",
    icon: "GridFour",
    href: "/tools/matrix-calculator",
    slug: "matrix-calculator",
    isNew: true,
    categoryId: "calculators",
  },
  {
    id: "55",
    title: "BMI Calculator",
    description: "Calculate Body Mass Index and health status.",
    icon: "ChartLineUp",
    href: "/tools/bmi-calculator",
    slug: "bmi-calculator",
    categoryId: "calculators",
  },
  {
    id: "101",
    title: "Split Bill Calculator",
    description:
      "Advanced bill splitting with custom shares, tax calculation, multi-currency support, PDF export, and QR code generation.",
    icon: "Receipt",
    href: "/tools/split-bill",
    slug: "split-bill",
    isNew: true,
    featured: true,
    categoryId: "calculators",
  },
  {
    id: "56",
    title: "Loan Calculator",
    description: "Calculate loan payments, interest, and amortization.",
    icon: "Coins",
    href: "/tools/loan-calculator",
    isComingSoon: true,
    categoryId: "calculators",
  },
  // =====================================================
  // SECURITY & PRIVACY - Tambahan
  // =====================================================
  {
    id: "68",
    title: "Password Strength Checker",
    description: "Analyze password security and get improvement suggestions.",
    icon: "Shield",
    href: "/tools/password-strength",
    slug: "password-strength",
    featured: true,
    isNew: true,
    categoryId: "security-privacy",
  },
  // Random String merged into Password Generator (similar intent)
  {
    id: "97",
    title: "Text Encryptor",
    description:
      "Securely encrypt and decrypt text using AES, DES, Rabbit, RC4, and customized RC4Drop algorithms.",
    icon: "LockKey",
    href: "/tools/text-encryptor",
    slug: "text-encryptor",
    categoryId: "security-privacy",
  },
  {
    id: "71",
    title: "HMAC Generator",
    description:
      "Generate HMAC signatures for API authentication and data verification.",
    icon: "KeyReturn",
    href: "/tools/hmac-generator",
    slug: "hmac-generator",
    featured: true,
    isNew: true,
    categoryId: "security-privacy",
  },
  {
    id: "46",
    title: "Image Steganography",
    description: "Hide secret text messages inside images invisibly.",
    icon: "Lock",
    href: "/tools/image-steganography",
    slug: "image-steganography",
    isNew: true,
    categoryId: "security-privacy",
  },

  // =====================================================
  // NETWORK & UTILITY (High Traffic)
  // =====================================================
  {
    id: "94",
    title: "My IP Address",
    description: "Check your public IP address, location, and ISP details.",
    icon: "Globe",
    href: "/tools/my-ip-address",
    slug: "my-ip-address",
    isComingSoon: true,
    categoryId: "web-seo",
  },
  {
    id: "95",
    title: "Internet Speed Test",
    description:
      "Test your internet connection speed (download, upload, ping).",
    icon: "ChartLineUp",
    href: "/tools/speed-test",
    slug: "speed-test",
    categoryId: "web-seo",
  },
  {
    id: "96",
    title: "WiFi QR Code Generator",
    description:
      "Create QR codes to share WiFi access without typing passwords.",
    icon: "WifiHigh",
    href: "/tools/wifi-qr-code",
    slug: "wifi-qr-code",
    categoryId: "media-images",
  },

  // =====================================================
  // DEVELOPER TOOLS - Tambahan
  // =====================================================

  // =====================================================
  // DESIGN TOOLS - Tambahan
  // =====================================================
  {
    id: "73",
    title: "Box Shadow Generator",
    description: "Create CSS box shadows with visual editor and copy code.",
    icon: "Cube",
    href: "/tools/box-shadow",
    slug: "box-shadow",
    isNew: true,
    categoryId: "design-tools",
  },
  {
    id: "74",
    title: "Border Radius Generator",
    description: "Generate custom CSS border radius with live preview.",
    icon: "Square",
    href: "/tools/border-radius",
    slug: "border-radius",
    isNew: true,
    categoryId: "design-tools",
  },
  {
    id: "75",
    title: "CSS Triangle Generator",
    description: "Create CSS triangles and arrows without images.",
    icon: "Triangle",
    href: "/tools/css-triangle",
    slug: "css-triangle",
    isNew: true,
    categoryId: "design-tools",
  },
  {
    id: "76",
    title: "Font Pairing Tool",
    description: "Find perfect Google Font combinations for your designs.",
    icon: "TextT",
    href: "/tools/font-pairing",
    slug: "font-pairing",
    isNew: true,
    categoryId: "design-tools",
  },
  {
    id: "77",
    title: "Color Blindness Simulator",
    description: "Test your designs for different types of color blindness.",
    icon: "Eye",
    href: "/tools/color-blindness",
    slug: "color-blindness",
    isNew: true,
    categoryId: "design-tools",
  },
  {
    id: "78",
    title: "CSS Flexbox Generator",
    description: "Visual flexbox layout generator with code export.",
    icon: "GridFour",
    href: "/tools/flexbox-generator",
    slug: "flexbox-generator",
    categoryId: "design-tools",
  },
  {
    id: "150",
    title: "Screen Ruler",
    description:
      "Measure objects in screenshots or screen captures with pixel rulers, boxes, calibration, and exportable results.",
    icon: "Ruler",
    href: "/tools/screen-ruler",
    slug: "screen-ruler",
    isNew: true,
    categoryId: "design-tools",
  },

  // =====================================================
  // TEXT UTILITIES - Tambahan
  // =====================================================
  {
    id: "79",
    title: "Text Reverser",
    description:
      "Reverse text, words, or letters. Perfect for creating mirror text.",
    icon: "ArrowsLeftRight",
    href: "/tools/text-reverser",
    slug: "text-reverser",
    isNew: true,
    categoryId: "text-utilities",
  },
  {
    id: "80",
    title: "Duplicate Line Remover",
    description: "Remove duplicate lines from text while preserving order.",
    icon: "FunnelSimple",
    href: "/tools/duplicate-remover",
    slug: "duplicate-remover",
    isNew: true,
    categoryId: "text-utilities",
  },

  // =====================================================
  // WEB & SEO - Tambahan
  // =====================================================
  {
    id: "110",
    title: "URL Encoder / Decoder",
    description:
      "Encode or decode URLs. Convert special characters to safer URL formats.",
    icon: "LinkSimple",
    href: "/tools/url-encoder",
    slug: "url-encoder",
    categoryId: "web-seo",
    isNew: true,
  },
  {
    id: "81",
    title: "Open Graph Generator",
    description: "Generate Open Graph tags for social media sharing.",
    icon: "ShareNetwork",
    href: "/tools/og-generator",
    slug: "og-generator",
    categoryId: "web-seo",
  },
  {
    id: "82",
    title: "Schema Markup Generator",
    description: "Create JSON-LD structured data for rich search results.",
    icon: "CodeSimple",
    href: "/tools/schema-generator",
    slug: "schema-generator",
    categoryId: "web-seo",
  },
  {
    id: "83",
    title: "Twitter Card Generator",
    description: "Generate XLogo Card meta tags for better link previews.",
    icon: "XLogo",
    href: "/tools/twitter-card",
    slug: "twitter-card",
    categoryId: "web-seo",
  },
  {
    id: "84",
    title: "UTM Builder",
    description: "Create UTM tracking parameters for campaign URLs.",
    icon: "LinkSimple",
    href: "/tools/utm-builder",
    slug: "utm-builder",
    categoryId: "web-seo",
  },
  {
    id: "85",
    title: "Link Shortener",
    description: "Create short, memorable URLs with custom aliases.",
    icon: "Link",
    href: "/tools/link-shortener",
    slug: "link-shortener",
    categoryId: "web-seo",
  },
  {
    id: "86",
    title: "HTTP Status Code Checker",
    description: "Check HTTP response codes and headers for any URL.",
    icon: "Globe",
    href: "/tools/http-status-checker",
    slug: "http-status-checker",
    categoryId: "web-seo",
  },
  {
    id: "87",
    title: "DNS Lookup",
    description: "Query DNS records (A, AAAA, MX, TXT, CNAME) for any domain.",
    icon: "HardDrives",
    href: "/tools/dns-lookup",
    slug: "dns-lookup",
    categoryId: "web-seo",
  },
  {
    id: "151",
    title: "IPv4 Subnet Calculator",
    description:
      "Calculate CIDR ranges, subnet masks, wildcard masks, broadcast addresses, and usable IPv4 hosts.",
    icon: "HardDrives",
    href: "/tools/ipv4-subnet-calculator",
    slug: "ipv4-subnet-calculator",
    isNew: true,
    categoryId: "web-seo",
  },
  {
    id: "126",
    title: "User Agent Checker",
    description:
      "Detect your browser, OS, device type, and rendering engine from your user agent string.",
    icon: "Globe",
    href: "/tools/user-agent-checker",
    slug: "user-agent-checker",
    isNew: true,
    categoryId: "web-seo",
  },

  // =====================================================
  // CALCULATORS - Tambahan
  // =====================================================
  {
    id: "88",
    title: "Tip Calculator",
    description: "Calculate tips and split bills among multiple people.",
    icon: "CurrencyDollar",
    href: "/tools/tip-calculator",
    slug: "tip-calculator",
    isComingSoon: true,
    categoryId: "calculators",
  },
  {
    id: "89",
    title: "Date Calculator",
    description: "Calculate differences between dates or add/subtract days.",
    icon: "CalendarDots",
    href: "/tools/date-calculator",
    slug: "date-calculator",
    isComingSoon: true,
    categoryId: "calculators",
  },
  {
    id: "90",
    title: "Compound Interest Calculator",
    description: "Calculate compound interest and investment growth over time.",
    icon: "TrendUp",
    href: "/tools/compound-interest",
    slug: "compound-interest",
    isComingSoon: true,
    categoryId: "calculators",
  },
  {
    id: "91",
    title: "Time Zone Converter",
    description: "Convert time between different time zones worldwide.",
    icon: "Clock",
    href: "/tools/timezone-converter",
    slug: "timezone-converter",
    isComingSoon: true,
    categoryId: "calculators",
  },
  {
    id: "92",
    title: "Grade Calculator",
    description: "Calculate weighted grades, GPA, and final scores.",
    icon: "GraduationCap",
    href: "/tools/grade-calculator",
    slug: "grade-calculator",
    isComingSoon: true,
    categoryId: "calculators",
  },

  // =====================================================
  // DEVELOPER TOOLS - Time Complexity Calculator
  // =====================================================
  {
    id: "128",
    title: "Tree Visualizer",
    description:
      "Build and animate binary trees from arrays or BST insertions. See interactive step-by-step traversal visualizations.",
    icon: "GitMerge",
    href: "/tools/tree-visualizer",
    slug: "tree-visualizer",
    isNew: true,
    featured: true,
    categoryId: "developer-tools",
  },
  // =====================================================
  // DEVELOPER TOOLS - Time Complexity Calculator
  // =====================================================
  {
    id: "121",
    title: "Time Complexity Calculator",
    description:
      "Analyze time and space complexity of your algorithm. Paste code or pseudocode and get instant Big O notation with detailed explanation.",
    icon: "ChartLineUp",
    href: "/tools/time-complexity-calculator",
    slug: "time-complexity-calculator",
    isNew: true,
    categoryId: "developer-tools",
  },

  // =====================================================
  // DEVELOPER TOOLS - Crontab Generator
  // =====================================================
  {
    id: "112",
    title: "Crontab Generator",
    description:
      "Build cron expressions visually or parse existing ones into human-readable schedules.",
    icon: "Timer",
    href: "/tools/crontab-generator",
    slug: "crontab-generator",
    isNew: true,
    categoryId: "developer-tools",
  },
  {
    id: "113",
    title: "Instant Visualizer",
    description:
      "Paste data from Excel or CSV and instantly get interactive charts, KPI cards, and insights — zero config needed.",
    icon: "ChartLineUp",
    href: "/tools/instant-visualizer",
    slug: "instant-visualizer",
    isNew: true,
    featured: true,
    categoryId: "developer-tools",
  },

  // =====================================================
  // MEDIA & IMAGES - Face Anonymizer
  // =====================================================
  {
    id: "124",
    title: "Face Anonymizer",
    description:
      "Automatically detect and blur or pixelate faces in photos. Select all or individual faces. 100% private — runs in your browser.",
    icon: "Scan",
    href: "/tools/face-anonymizer",
    slug: "face-anonymizer",
    isNew: true,
    categoryId: "media-images",
  },
  // DEVELOPER TOOLS - HTML Viewer
  // =====================================================
  {
    id: "117",
    title: "HTML Viewer",
    description:
      "Paste HTML, CSS, and JavaScript and see the live rendered result instantly. Manage separate files or write everything in a single document.",
    icon: "CodeSimple",
    href: "/tools/html-viewer",
    slug: "html-viewer",
    isNew: true,
    categoryId: "developer-tools",
  },
  // =====================================================
  // MEDIA & IMAGES - Image Annotator
  // =====================================================
  {
    id: "125",
    title: "Image Annotator",
    description:
      "Annotate images with arrows, shapes, text, and freehand drawing. Upload or paste an image and download the result.",
    icon: "NotePencil",
    href: "/tools/image-annotator",
    slug: "image-annotator",
    isNew: true,
    categoryId: "media-images",
  },
  // =====================================================
  // DEVELOPER TOOLS - P2P Local Data Transfer
  // =====================================================
  {
    id: "120",
    title: "P2P Local Data Transfer",
    description:
      "Send text and files directly between devices via WebRTC — no uploads, no accounts, nothing stored on any server.",
    icon: "ShareNetwork",
    href: "/tools/p2p-share",
    slug: "p2p-share",
    isNew: true,
    categoryId: "developer-tools",
  },
  // =====================================================
  // CALCULATORS - Hypothesis Test
  // =====================================================
  {
    id: "122",
    title: "Statistical Hypothesis Test",
    description:
      "Calculate hypothesis tests for research: t-test, z-test, chi-square, ANOVA, and Pearson correlation. Paste data directly from Excel.",
    icon: "MagicWand",
    href: "/tools/hypothesis-test",
    slug: "hypothesis-test",
    isNew: true,
    categoryId: "calculators",
  },
  // =====================================================
  // DEVELOPER TOOLS - Chmod Calculator
  // =====================================================
  {
    id: "129",
    title: "Chmod Calculator",
    description:
      "Calculate Unix file permissions interactively. Toggle read, write, execute bits for owner, group, and others. Get octal, symbolic, and chmod command output instantly.",
    icon: "Shield",
    href: "/tools/chmod-calculator",
    slug: "chmod-calculator",
    isNew: true,
    categoryId: "developer-tools",
  },
  {
    id: "133",
    title: "UUID Collision Calculator",
    description:
      "Calculate the probability of generating duplicate UUIDs, ULIDs, or NanoIDs using the Birthday Problem formula.",
    icon: "ArrowsLeftRight",
    href: "/tools/uuid-collision-calculator",
    slug: "uuid-collision-calculator",
    isNew: true,
    categoryId: "security-privacy",
  },
  // =====================================================
  // TEXT UTILITIES - Unicode Analyzer
  // =====================================================
  {
    id: "132",
    title: "Unicode Analyzer",
    description:
      "Inspect every character: code points (U+XXXX), UTF-8 bytes, categories, Unicode blocks, and normalization forms. Detects invisible and special characters.",
    icon: "Scan",
    href: "/tools/unicode-analyzer",
    slug: "unicode-analyzer",
    isNew: true,
    categoryId: "text-utilities",
  },
  {
    id: "155",
    title: "Linebreak Swap",
    description:
      "Convert whitespace to newlines and vice versa. Split inline text into lines or join lines into one.",
    icon: "ArrowsDownUp",
    href: "/tools/linebreak-swap",
    slug: "linebreak-swap",
    isNew: true,
    categoryId: "text-utilities",
  },
  {
    id: "156",
    title: "Text Sorter",
    description:
      "Sort lists with 10 modes: alphabetical, natural, numeric, by length, reverse, and shuffle. Remove duplicates and pick any separator.",
    icon: "SortAscending",
    href: "/tools/text-sorter",
    slug: "text-sorter",
    isNew: true,
    categoryId: "text-utilities",
  },
  {
    id: "157",
    title: "Binary Converter",
    description:
      "Convert text to binary code or decode binary back to readable text. Supports ASCII, Unicode, emoji, and multiple byte separators.",
    icon: "CodeSimple",
    href: "/tools/binary-converter",
    slug: "binary-converter",
    isNew: true,
    categoryId: "text-utilities",
  },
];

// Search suggestions for the search bar
export const SEARCH_SUGGESTIONS = [
  { type: "tool" as const, name: "GitMerge PDF", category: "Documents" },
  {
    type: "tool" as const,
    name: "Image Compressor",
    category: "Media & Images",
  },
  {
    type: "tool" as const,
    name: "Password Generator",
    category: "Security & Privacy",
  },
  {
    type: "tool" as const,
    name: "QR Code Generator",
    category: "Media & Images",
  },
  {
    type: "tool" as const,
    name: "JSON Formatter",
    category: "Developer Tools",
  },
  { type: "tool" as const, name: "Color Picker", category: "Design Tools" },
  {
    type: "tool" as const,
    name: "ASCII Table Generator",
    category: "Text Utilities",
  },
  { type: "tool" as const, name: "Text Affixer", category: "Text Utilities" },
  { type: "tool" as const, name: "Teleprompter", category: "Text Utilities" },
  { type: "tool" as const, name: "Running Text", category: "Media & Images" },
  { type: "tool" as const, name: "Compare PDF", category: "Documents" },
  { type: "tool" as const, name: "Screen Ruler", category: "Design Tools" },
  {
    type: "tool" as const,
    name: "Aspect Ratio Calculator",
    category: "Calculators",
  },
  {
    type: "tool" as const,
    name: "IPv4 Subnet Calculator",
    category: "Web & SEO",
  },
  { type: "category" as const, name: "PDF & Documents", count: 10 },
  { type: "category" as const, name: "Media & Images", count: 12 },
  { type: "category" as const, name: "Developer Tools", count: 13 },
];
