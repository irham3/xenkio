...
import { CaretLeft, CaretRight, VideoCamera, VideoCameraSlash, WifiHigh, Eye, EyeSlash } from '@phosphor-icons/react/dist/ssr';
            ...
            >
                <div className="relative rounded-2xl overflow-hidden shadow-2xl border-2 border-white/80 bg-black">
                    ...
                    <div className="absolute top-2 left-2 right-2 flex items-center justify-between">
                        ...
                        >
                            <EyeSlash className="w-3 h-3"  weight="duotone"/>
                        </button>
                    </div>
                        ...
                        {isModelLoading ? (
                            <>
                                <div className="w-5 h-5 rounded-lg bg-amber-50 flex items-center justify-center animate-spin">
                                    <WifiHigh className="w-3 h-3 text-amber-600"  weight="duotone"/>
                                </div>
                            ...
                            <>
                                <div className="relative">
                                    <div className={`w-5 h-5 rounded-lg flex items-center justify-center transition-colors duration-300 ${isHandDetected ? 'bg-primary-100' : 'bg-gray-100'}`}>
                                        <VideoCamera className={`w-3 h-3 transition-colors ${isHandDetected ? 'text-primary-600' : 'text-gray-400'}`}  weight="duotone"/>
                                    </div>
                                    {isHandDetected && (
                                ...
                                >
                                    {showCamera ? <Eye className="w-3 h-3"  weight="duotone"/> : <EyeSlash className="w-3 h-3"  weight="duotone"/>}
                                </button>
                            </>
                        )}
                    </div>
            ...
            {error && (
                <div className="fixed bottom-24 right-6 z-50 animate-in fade-in slide-in-from-bottom-2">
                    <div className="flex items-center gap-2.5 px-3 py-2 bg-red-50 backdrop-blur-md rounded-2xl shadow-xl border border-red-100">
                        <VideoCameraSlash className="w-3.5 h-3.5 text-red-600"  weight="duotone"/>
                        ...
                        >
                            <div className="w-20 h-20 rounded-full bg-white/10 flex items-center justify-center mb-2">
                                {gestureDirection === 'left' ? (
                                    <CaretRight className="w-10 h-10 animate-bounce-x"  weight="duotone"/>
                                ) : (
                                    <CaretLeft className="w-10 h-10 animate-bounce-x-reverse"  weight="duotone"/>
                                )}
                            </div>
